<template>
  <section class="w-full flex flex-col h-screen bg-[#151424]">
    ...
    >
      ...
      <h1 class="text-textLight1 text-lg w-full text-center">
        Payment history
      </h1>
    </div>
    ...
    <article class="flex-1 py-6 px-4">
      <div
        class="grid grid-cols-1 gap-5 px-4 md:grid-cols-3 md:px-10 mx-auto text-gray-300"
        v-if="recentDeposits.length"
      >
        <div
          class="flex justify-between items-center gap-3 p-2 bg-primaryDark2 rounded-md"
        ...
        >
          ...
          <p class="font-medium text-sm"><span>$</span>{{ deposit.amount }}</p>
          <p class="text-sm">
            {{ deposit.status ? "successful" : "pending.." }}
          </p>
        </div>
      </div>
      <div class="items-center justify-center h-full flex text-white" v-else>
